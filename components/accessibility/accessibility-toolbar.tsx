...
export function AccessibilityToolbar() {
  const { settings, updateSetting, resetSettings, isToolbarOpen, setIsToolbarOpen } = useAccessibility()
  ...
  const cycleCursorSize = () => {
    ...
    const currentIndex = sizes.indexOf(settings.cursorSize)
    const nextIndex = (currentIndex + 1) % sizes.length
    updateSetting("cursorSize", sizes[nextIndex])
  }
  ...
  const getContrastLabel = () => {
    const labels = { normal: "Contrast +", invert: "Invert Colors", dark: "Dark Contrast", light: "Light Contrast" }
  ...
  const getDyslexiaLabel = () => {
    const labels = { normal: "Dyslexia Friendly", dyslexia: "Dyslexia Font", legible: "Legible Font" }
  ...
  const getCursorLabel = () => {
    const labels = { normal: "Cursor", large: "Large Cursor", "extra-large": "XL Cursor" }
    return labels[settings.cursorSize]
  }
  ...
  const toolbarContent = (
    <div className="accessibility-toolbar-panel fixed top-0 right-0 h-full w-80 bg-white shadow-2xl flex flex-col" style={{ zIndex: 2147483646 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3.5 bg-gradient-to-r from-[#38A1E5] to-[#7284FF] text-white flex-shrink-0">
        ...
        <div className="flex-1 flex flex-col p-4 bg-white overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          <div className="grid grid-cols-2 gap-3 flex-1">
            ...
            <AccessibilityOption
              ...
              activeIndex={["normal", "left", "center", "right", "justify"].indexOf(settings.textAlign)}
            />
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-[#38A1E5] to-[#7284FF] hover:from-[#2E8BD1] hover:to-[#5F6FE8] text-white py-3 mt-4 shadow-lg flex-shrink-0"
          >
