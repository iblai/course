...
# Pin Node 25.3.0 via `n` (matches the rest of the ibl.ai SPA stack).
...
# Build the standalone output (requires `output: 'standalone'` in
# next.config.mjs — courseai sets this).
RUN pnpm run build
...
# Install Node 25.3.0 from the official musl build (Alpine) so the
# runtime matches the build stage.
